...
import static rp.plf.MesDemandesDeCombatsActivity.generateRandomString;
...
public class PartagerTeamActivity extends AppCompatActivity {
    ...
    FirebaseAuth auth;
    Button validerButt;
    EditText nomEquipe,lien,paste;
    String nomEquipeS,lienS,pasteS;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_partager_team);
        ...
        validerButt = findViewById(R.id.validerButt);
        nomEquipe = findViewById(R.id.nomEquipe);
        lien = findViewById(R.id.lien);
        paste = findViewById(R.id.paste);


        validerButt.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                nomEquipeS = nomEquipe.getText().toString();
                lienS = lien.getText().toString();
                pasteS = paste.getText().toString();
                if(nomEquipeS.isEmpty() || lienS.isEmpty() || pasteS.isEmpty()){
                    Toast.makeText(PartagerTeamActivity.this, "Merci de remplir tout les champs", Toast.LENGTH_SHORT).show();
                }else{
                    ajouterTeamEnBase(nomEquipeS, lienS, pasteS);
                }
            }
        });
...
//        runOnUiThread(thread);
    ...
    private void ajouterTeamEnBase(String nomEquipeS, String lienS, String pasteS) {
        FirebaseDatabase database = FirebaseDatabase.getInstance();
        DatabaseReference usersRef = database.getReference("ShowdownTeam");
        TeamRP team = new TeamRP(nomT.getText().toString(),pasteS, lienS,nomEquipeS);
        String monId = generateRandomString();
        usersRef.child(monId).setValue(team);
        Toast.makeText(this, "Team publiée avec succès", Toast.LENGTH_SHORT).show();
        Intent intent = new Intent(getApplicationContext(), GestionAreneActivity.class);
        startActivity(intent);
        finish();
    }
