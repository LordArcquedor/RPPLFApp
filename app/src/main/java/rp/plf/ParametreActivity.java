...
public class ParametreActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        usersRef.addListenerForSingleValueEvent(new ValueEventListener() {
            ...
            public void onDataChange(DataSnapshot dataSnapshot) {
                for (DataSnapshot userSnapshot : dataSnapshot.getChildren()) {
                    ...
                    lespseudo.add(pseudoSnapshot.getValue(String.class));
                }
            }
        ...
        conf2.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                update = newpseudo.getText().toString();
                if(update.isEmpty()){
                    Toast.makeText(ParametreActivity.this, "Le pseudo ne doit pas être vide.", Toast.LENGTH_SHORT).show();
                }else if(update.length()<3){
                    Toast.makeText(ParametreActivity.this, "Le pseudo doit contenir au minimum 3 caractères.", Toast.LENGTH_SHORT).show();
                }else{
                    String update = newpseudo.getText().toString();
                    if(lespseudo.contains(update)){
                        Toast.makeText(ParametreActivity.this, "Désolé, pseudo déjà pris", Toast.LENGTH_SHORT).show();
                    }else{
                        FirebaseDatabase database = FirebaseDatabase.getInstance();
                        DatabaseReference usersRef = database.getReference("utilisateurs");
                        ...
                        query.addValueEventListener(new ValueEventListener(){
                            ...
                            public void onDataChange(@NonNull DataSnapshot snapshot) {
                                modifierEnBaseBanniere(user.getUid(), update);
                                Toast.makeText(ParametreActivity.this, "Votre pseudo a bien été modifiée", Toast.LENGTH_SHORT).show();
                                Intent intent = new Intent(getApplicationContext(), MainActivity.class);
                                startActivity(intent);
                                finish();
                            }
    ...
    private void modifierEnBaseBanniere(String pseudo, String lienBann) {
        ...
        DatabaseReference usersRef = database.getReference("utilisateurs");
        if(!lienBann.isEmpty()){
            usersRef.child(pseudo).child("pseudo").setValue(lienBann);
            Intent intent = new Intent(getApplicationContext(),ProfilActivity.class);
            startActivity(intent);
            finish();

        }else{
            Toast.makeText(ParametreActivity.this, "Le contenu ne peut pas être vide.", Toast.LENGTH_SHORT).show();
        }
