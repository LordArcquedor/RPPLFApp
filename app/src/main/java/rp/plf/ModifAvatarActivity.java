...
public class ModifAvatarActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_modif_avatar);
                            ...
                            query.addValueEventListener(new ValueEventListener() {
                                ...
                                public void onDataChange(DataSnapshot dataSnapshot) {
                                    for (DataSnapshot avatarsSnapshot : dataSnapshot.getChildren()) {
                                        ...
                                        if (utilisateur != null) {
                                            listeAvatar.addAll(utilisateur.getMesAvatars());
                                            for(int i =0; i<listeAvatar.size();i++){
                                                Linear3 = findViewById(R.id.Linear3);
                                                ImageView imageView = new ImageView(getApplicationContext());
                                                String lienBan = listeAvatar.get(i);
                                                int desiredWidth = 450;
                                                int desiredHeight = 450;

                                                Picasso.get()
                                                        .load(listeAvatar.get(i))
...
//
                                                ...
                                                imageView.setOnClickListener(new View.OnClickListener() {
                                                    ...
                                                    public void onClick(View v) {
                                                        modifierEnBaseAvatar(user.getUid(),lienBan);
                                                        Toast.makeText(ModifAvatarActivity.this, "Votre avatar a bien été modifié", Toast.LENGTH_SHORT).show();
                                                        Intent intent = new Intent(getApplicationContext(), ProfilActivity.class);
                                                        startActivity(intent);
                                                        finish();
                                                    }
...
//        thread.start();
    ...
    private void modifierEnBaseAvatar(String pseudo, String lienBann) {
        FirebaseDatabase database = FirebaseDatabase.getInstance();
        DatabaseReference usersRef = database.getReference("utilisateurs");
        if(!lienBann.isEmpty()){
            usersRef.child(pseudo).child("photo").setValue(lienBann);
            Intent intent = new Intent(getApplicationContext(),ProfilActivity.class);
            startActivity(intent);
            finish();

        }else{
            Toast.makeText(ModifAvatarActivity.this, "Le contenu ne peut pas être vide.", Toast.LENGTH_SHORT).show();
        }
